	...
	async def join_room(self, room_name):
		if room_name in self.rooms:
			if len(self.rooms[room_name]) < 3:
				...
				player_number = len(self.rooms[room_name])  # Assign player number based on the order of joining

				await self.send(text_data=json.dumps({
					'message': 'Joined room',
					'room_name': room_name,
					'player_uuid': self.player_uuid,
					'player_number': player_number  # Send player number to client
				}))
				await self.channel_layer.group_add(
					self.room_name,
					self.channel_name
				...
				await self.channel_layer.group_send(
					self.room_name,
					{
						...
						'player_number': player_number  # Broadcast player number to the group
					}
				)
			else:
				await self.send(text_data=json.dumps({
